...
NetApp Cloud Volumes ONTAP 스토리지는 AWS, Azure 및 Google Cloud의 컨테이너 배포 환경에 데이터 보호, 안정성 및 유연성을 제공합니다. Trident는 고객의 상태 저장 애플리케이션에 필요한 영구 Cloud Volumes ONTAP 스토리지를 활용하는 동적 스토리지 프로비저너 역할을 합니다. Trident Protect는 데이터 보호, 마이그레이션 및 비즈니스 연속성과 같은 상태 저장 애플리케이션의 데이터 관리 요구 사항에 사용할 수 있습니다.
